...
int dropBlockOnDisk(char * flag, unsigned int addr)
{
    char filename[40];

    sprintf(filename, "%s_%d.blk", flag, addr);
...
unsigned char *readBlockFromDisk(char * flag, unsigned int addr, Buffer *buf)
{
    char filename[40];
    ...
    if (!fp)
    {
        //perror("Reading Block Failed! %s_%u.blk\n", flag, addr);
        printf("Reading Block Failed! %s_%u.blk\n", flag, addr);
        return NULL;
    }
...
int writeBlockToDisk(unsigned char *blkPtr, char * flag, unsigned int addr, Buffer *buf)
{
    char filename[40];
